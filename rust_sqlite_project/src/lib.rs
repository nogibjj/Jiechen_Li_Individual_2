...
pub fn setup_db() -> Result<()> {
    let conn = Connection::open("spotify_2023_1_new.db")?;
...
pub fn query_tracks() -> Result<()> {
    ...
    log_query("SELECT artist_name, in_spotify_playlists, in_apple_playlists, bpm FROM tracks;", LOG_FILE);
...
pub fn delete_track(artist_name: &str) -> Result<()> {
    ...
    log_query(&format!("DELETE FROM tracks WHERE artist_name = '{}';", artist_name), LOG_FILE);
